...
import { getFormattedDate, getPermalink,  } from "@/utils/content";
...
return <article className={`max-w-md mx-auto md:max-w-none grid gap-6 md:gap-8 ${post.featuredImage ? 'md:grid-cols-2' : ''}`}>
  {
    (img && post.slug ) && (
      <Link className="relative block group" href={getPermalink(post.slug, 'Post', lang)}>
  ...
  <div className="mt-2">
    <header>
      ...
      <h2 className="text-xl sm:text-2xl font-bold leading-tight mb-2 font-heading dark:text-slate-300">
        <Link className="hover:text-primary dark:hover:text-blue-700 transition ease-in duration-200" href={getPermalink(post.slug || '', 'Post', lang)}>
          {post.title}
        </Link>
      </h2>
    </header>
