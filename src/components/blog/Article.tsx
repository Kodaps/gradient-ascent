      ...
      <article>
        {!hideHeader && (
          <header className={post.featuredImage ? "text-center" : ""}>
            <p className="mx-auto max-w-5xl px-4 sm:px-6">
              ...
              {post.author?.name && (
                <Link
                  href={getPermalink(post.author.slug, "Person", lang)}
                  className="mx-2"
                >
                  {post.author?.name}{" "}
                </Link>
            ...
            <div className="flex flex-row sm:px-6 prose-md container prose-stone mx-auto mt-8 max-w-3xl px-6 prose-headings:font-bold prose-headings:tracking-tighter prose-a:text-primary-600 prose-image:rounded-md prose-image:shadow-lg  dark:prose-invert dark:prose-headings:text-slate-300 dark:prose-a:text-primary-400 ">
              {parsedContent?.lead ? (
                <p
                  className="flex-grow dark:text-slate-400 text-lg"
                  dangerouslySetInnerHTML={{
                    __html: renderMarkdown(parsedContent?.lead),
                  }}
                />
              ) : (
                <p className="flex-grow dark:text-slate-400 text-lg">
                  {post.description}
                </p>
              )}
            </div>
            {post.video ? (
              <div className="max-w-3xl mx-auto">
                <YouTubeEmbed className="w-full" embedId={post.video} />
              </div>
            ) : (
              <>
                {img ? (
                  <>
                    ...
                    {post.credits && (
                      <p
                        ...
                        dangerouslySetInnerHTML={{ __html: post.credits }}
                      />
                    )}
                  </>
                ) : (
        ...
        <div className="prose prose-lg prose-headings:font-heading prose-headings:leading-tighter container prose-stone mx-auto mt-8 max-w-3xl px-6 prose-headings:font-bold prose-headings:text-xl prose-headings:tracking-tighter prose-a:text-primary-600 prose-image:rounded-md prose-image:shadow-lg  dark:prose-invert dark:prose-headings:text-slate-300 dark:prose-a:text-primary-400 ">
          {parsedContent?.intro && (
            <div
              dangerouslySetInnerHTML={{
                __html: renderMarkdown(parsedContent?.intro),
              }}
            />
          )}

          {parsedContent?.takeaway && (
            <div
              className={`mb-2 rounded-md border border-gray-300 shadow-md md:px-6 py-4 md:py-5 px-5`}
            >
              ...
              <p
                dangerouslySetInnerHTML={{
                  __html: renderMarkdown(parsedContent?.takeaway),
                }}
              />
            </div>
          )}
          ...
          {parsedContent?.sections.map((section, index) => (
            <ArticleSection key={index} paragraph={section} lang={lang} />
          ))}
        </div>
      </article>
    </section>
