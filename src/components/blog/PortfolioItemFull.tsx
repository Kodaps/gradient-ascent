...
import { Person, Portfolio } from "contentlayer/generated";
...
export const PortfolioItemFull:React.FC<PortfolioItemFullProps> = ({item, lang, type, hideHeader}) => {
  ...
  <article>
     <header className={item.image ? 'text-center' : ''}>
      <p className="mx-auto max-w-5xl px-4 sm:px-6">
        { (item.date && (type!=='Person')) && <time dateTime={item.date}>{(item.date)}</time>}
      ...
      <h1 className="leading-tighter font-heading mx-auto mb-8 max-w-3xl px-4 text-4xl font-bold tracking-tighter sm:px-6 md:text-5xl">
        ...
        />}
        { item.prompt && <p className="text-muted dark:text-slate-400 text-lg" dangerouslySetInnerHTML={{__html: item.prompt }} />}
    </header>
  </article>
</section>;
}
