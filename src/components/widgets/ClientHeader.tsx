    ...
    >
      <div className="mx-auto w-full max-w-5xl py-3 px-3 md:flex md:justify-between md:py-3.5 md:px-4">
        ...
        <nav
                ...
                <li key={`item-link-${index}`} className={links?.length ? 'dropdown' : ''}>
                  {links && links.length ? (
                    <>
                      ...
                      >
                        { `menu.${label}` } {icon && <Icon name={icon} className="ml-0.5 hidden h-3.5 w-3.5 md:inline" />}
                      </button>
                      <ul
                        className={`${
          ...
          { isFeatureActive('menu') && <FullNavigationMenu lang={lang} blogLinks={blogLinks} className="items-center" />}
          </nav>
