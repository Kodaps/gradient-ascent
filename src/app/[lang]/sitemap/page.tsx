...
import { allPagesByLang, allPostsByLang, getPermalink } from '@/utils/content';
...
export async function generateMetadata( {params: {lang}}: SitemapParams ):Promise<Metadata> {
...
export default async function SitemapPage({ params: { lang } }: SitemapParams) {
  const t = await getTranslations(lang);
  const otherLang = lang === 'en' ? 'fr' : 'en';
  const _posts = allPostsByLang(lang);
  const _pages = allPagesByLang(lang);
  ...
  const categories = Object.entries(count.category)
    .sort(([itmA, countA], [itmB, countB]) => (countB as number) - (countA as number))
    ...
    <section className="">
      ...
      <div className="px-4 py-16 mx-auto max-w-5xl lg:py-20">
        <div className="container prose">
          ...
          <Link href="/">{t('home.title')}</Link><br/>
          { _pages.map((page) => {
            return <span key={page.slug}><Link  href={getPermalink(page.slug || '', 'Page', lang)}>{page.title}</Link><br/></span>;
          })}
          <h2>{t('blog.h1')}</h2>
          ...
          {categories.map((category) => {
            return (
              <>
                <Link key={category} href={getPermalink(category, 'category', lang)}>
                ...
                <ul>
                  {_posts
                    ...
                    .map((post) => {
                      return (
                        <li key={post.slug}>
                          <Link href={getPermalink(post.slug || '', 'Post', lang)}>{post.title}</Link>
                        </li>
                      );
                    })}
                </ul>
          ...
          <h3>{t('blog.tags')}</h3>
          {tags.map((tag) => {
            return <span key={tag}><Link  href={getPermalink(tag, 'tag', lang)}>
                  {t('blog.tagged') + ' ' + t(tag)}
                </Link><br/>
                </span>;
          })}
