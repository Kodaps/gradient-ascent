...
const sortByDate = (
  post1: ContentDocumentTypes,
  post2: ContentDocumentTypes
) => {
...
export const findDocumentById = (id: string) => {
  ...
  return allDocumentsUnsorted.find(
    (item) => unHyphenate(item.notionId) == idNoHyphen
  )
}
...
const allPortfolios = allPortfoliosUnsorted
  .sort(sortByDate)
...
export const getPermalinkByCoreSlug = (lang: Lang, coreSlug: string) => {
  ...
  const page = allDocumentsByLang("Page", lang).find(
    (item) => item.coreSlug == coreSlug
  )
  if (page) {
    return `/${lang}/${page.slug}`
  }

  const blog = allDocumentsByLang("Post", lang).find(
    (item) => item.coreSlug == coreSlug
  )
  if (blog) {
...
export const generateContentMetaData = (
  ...
  const data: Metadata = { title: post?.title }
...
export const findLatestPortfolio = async (count?: number, page?: number) => {
  ...
  return items
    ? items.sort(sortByDate).slice(_page * _count, (_page + 1) * _count)
    : []
}
...
export const findAllCategories = async (lang: Lang) => {
  const items = allPostsByLang(lang)
  const data = items ? items.map((item) => slugify(item.category)) : []
  return data
...
export const findLatestPostsByTag = async (
  ...
  return items
    ? items
        .filter((item) => !!item)
        .filter(
          (item) => (item.tags || []).indexOf(_tag) > -1 && item.lang == lang
        )
        .sort(sortByDate)
        .slice(_page * _count, (_page + 1) * _count)
    : []
...
export const parseContent = (article: string): PostContent => {
  const FAQ = article.split("\n## Frequently Asked Questions")

  const _article = FAQ[0]
  ...
  const sections = _article.split("\n## ")

  const firstSection = sections.shift()

  const items = (firstSection || "").split("\n### ")
  ...
  const introBits = items.join("\n ### ").split("\n")

  const lead = introBits.shift() || ""
...
const augmentDocument = (doc?: DocumentTypes) => {
  ...
  const translation = doc.translation[0]
  ...
  const alt = {
    [otherDoc.lang]: otherDoc.slug,
    _id: otherDoc._id,
    type,
    _raw: { ...otherDoc._raw, type },
  }
  doc.alts.push(alt)
